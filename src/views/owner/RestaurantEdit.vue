<template>
  <Layout>
    <template v-slot="">
      <h1 class="text-2xl font-bold text-center mt-4">Update Restaurant</h1>

      <div class="px-10 mx-auto mt-10">
        <form @submit.prevent="formSubmit" class="flex flex-col w-full" enctype="multipart/form-data">
          <div class="flex justify-around">
            <div class="w-2/5">
              <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Restaurant name <span
              ...
              <label for="business_licence" class="block mb-2 text-sm font-medium text-gray-900">Business licence <span
                  class="text-red-600">*</span></label>
              <input type="file" id="business_licence" @change="
                onFileChange(
                  $event,
                ...
                "
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
              <span v-if="errors.business_licence" class="text-red-600 text-xs">{{ errors.business_licence[0] }}</span>
              <img v-if="businessLicencePreview" :src="businessLicencePreview" alt="Logo" width="100" />
              <label for="vat_certificate" class="block mb-2 text-sm font-medium text-gray-900">VAT certificate <span
                  class="text-red-600">*</span></label>
              <input type="file" id="vat_certificate"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                accept="image/*" @change="
                  onFileChange(
                    $event,
                  ...
                  " />
              <span v-if="errors.vat_certificate" class="text-red-600 text-xs">{{ errors.vat_certificate[0] }}</span>
              <img v-if="vatCertificatePreview" :src="vatCertificatePreview" alt="Logo" width="100" />
              <label for="tax_identification_number" class="block mb-2 text-sm font-medium text-gray-900">Tax
                identification number
                <span class="text-red-600">*</span></label>
              <input v-model="form.tax_identification_number" type="number" id="tax_identification_number"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
              <span v-if="errors.tax_identification_number" class="text-red-600 text-xs">{{
                errors.tax_identification_number[0] }}</span>
              <label for="restaurant_menu" class="block mb-2 text-sm font-medium text-gray-900">Updated restaurant menu
                <span class="text-red-600">*</span></label>
              <input type="file" id="restaurant_menu" accept="image/*" @change="
                onFileChange(
                  $event,
                ...
                "
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
              <span v-if="errors.restaurant_menu" class="text-red-600 text-xs">{{ errors.restaurant_menu[0] }}</span>
              <img v-if="restaurantMenuPreview" :src="restaurantMenuPreview" alt="Logo" width="100" />
            </div>
            <div class="w-2/5">
              <label for="logo" class="block mb-2 text-sm font-medium text-gray-900">Logo</label>
              <input type="file" id="logo" @change="onFileChange($event, 'logoPreview', 'logo')" accept="image/*"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
              <span v-if="errors.logo" class="text-red-600 text-xs">{{
                errors.logo[0]
              }}</span>
              <img v-if="logoPreview" :src="logoPreview" alt="Logo" width="100" />
              <label for="address" class="block mb-2 text-sm font-medium text-gray-900">Full address <span
                  class="text-red-600">*</span></label>
              ...
              <label for="bank_statement" class="block mb-2 text-sm font-medium text-gray-900">Bank statement <span
                  class="text-red-600">*</span></label>
              <input name="bank_statement" type="file" id="bank_statement" @change="
              ...
              <img v-if="bankStatementPreview" :src="bankStatementPreview" alt="Logo" width="100" />

              <label for="utility_bill" class="block mb-2 text-sm font-medium text-gray-900">Utility bill <span
                  class="text-red-600">*</span></label>
              <input type="file" id="utility_bill" @change="
              ...
              <img v-if="utilityBillPreview" :src="utilityBillPreview" alt="Logo" width="100" />
            </div>
          </div>
...
export default {
  ...
  methods: {
    async fetchDetails() {
      try {
        const res = await this.axios.get(
          `/api/owner/restaurant/${this.$route.params.id}`
        );
        ...
        this.form.tax_identification_number =
          this.restaurant.tax_identification_number;
        this.logoPreview = this.restaurant.logo;
        this.businessLicencePreview = this.restaurant.business_licence;
        this.vatCertificatePreview = this.restaurant.vat_certificate;
        this.bankStatementPreview = this.restaurant.bank_statement;
        this.utilityBillPreview = this.restaurant.utility_bill;
        this.restaurantMenuPreview = this.restaurant.restaurant_menu;
      } catch (error) {
    ...
    validateForm() {
      ...
      // validate logo
      if (this.form.logo) {
        if (!this.validFileTypes.includes(this.form.logo.type)) {
          isValid = false;
      ...
      if (this.form.tax_identification_number) {
        if (this.form.tax_identification_number.toString().length != 10) {
          isValid = false;
      ...
      if (this.form.bank_statement) {
        if (!this.validFileTypes.includes(this.form.bank_statement.type)) {
          isValid = false;
      ...
      if (this.form.utility_bill) {
        if (!this.validFileTypes.includes(this.form.utility_bill.type)) {
          isValid = false;
    ...
    async formSubmit() {
      try {
        if (this.validateForm()) {
          ...
          const res = await this.axios.post(`/owner/restaurant/update/${this.$route.params.id}`, this.form, {
            headers: {
